...
addEventHandler("onClientRender",root,function()
	...
	if (room == "menu") then
		...
		if isCursorOnArea(0,0,sH*0.400,sH*0.200) then image = playimage2;color = tocolor(200,200,200,150); end
		dxDrawImage(0,0,sH*0.400,sH*0.200,image,0,0,0,tocolor(255,255,255));
		dxDrawImage(sW*0.99-sH*0.200,sH*0.99-sH*0.115,sH*0.200,sH*0.100,"images/hud/logo.png",0,0,0,tocolor(255,255,255));
		dxDrawText("ALPHA 1",sW*0.99-sH*0.200,sH*0.99-sH*0.115,sW*0.99-sH*0.200+sH*0.200,sH*0.99-sH*0.115+sH*0.100,tocolor(250,200,25),1*sH*0.001,"default-bold","center","bottom",false,false,false,false,false,0,0,0);
		...
		if isCursorOnArea(sW*0.4,sH*0.8,dxGetTextWidth("<",5*sH*0.001,"default-bold"),dxGetFontHeight(5*sH*0.001,"default-bold")) then color1 = tocolor(0,0,0); end
		if isCursorOnArea(sW*0.58,sH*0.8,dxGetTextWidth(">",5*sH*0.001,"default-bold"),dxGetFontHeight(5*sH*0.001,"default-bold")) then color2 = tocolor(0,0,0); end
		...
		dxDrawText(translateLocalization("play"),sH*0.070,sH*0.005,sH*0.200,sH*0.100,color,playTextScale,bebasneue,"left","center",false,false,false,false,false,0,0,0);
		dxDrawText("<",sW*0.4,sH*0.8,0,0,color1,5*sH*0.001,"default-bold","left","top",false,false,false,false,false,0,0,0);
		dxDrawText(">",sW*0.58,sH*0.8,0,0,color2,5*sH*0.001,"default-bold","left","top",false,false,false,false,false,0,0,0);
		-- lang selection
		local langcolor = tocolor(255,255,255);
		if (currLang == "en") then currLang = "";
		elseif (currLang == "lv") then currLang = "";
		elseif (currLang == "ru") then currLang = "";
		end
...
bindKey("mouse1","down",function()
	...
	local room = getElementData(localPlayer,"room");
	if (room == "menu") then
		if (isCursorOnArea(0,0,sH*0.400,sH*0.200)) then
			local skin = getElementModel(ped);
			triggerServerEvent("lobby",localPlayer,skin);
			showCursor(false);
		elseif (isCursorOnArea(sW*0.4,sH*0.8,dxGetTextWidth("<",5,"default-bold"),dxGetFontHeight(5,"default-bold"))) then
		...
		elseif (isCursorOnArea(sW*0.58,sH*0.8,dxGetTextWidth(">",5,"default-bold"),dxGetFontHeight(5,"default-bold"))) then
			local currSkin = getElementModel(ped);
			local newSkin;
			for i=#skins,1,-1 do
				local skin = skins[i];
